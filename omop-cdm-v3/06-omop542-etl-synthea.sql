...
CREATE MATERIALIZED VIEW person 
AS
SELECT
  hash(ID) AS PERSON_ID -- TO DO: replace this with a better method
...
CREATE MATERIALIZED VIEW IP_VISITS AS (
...
CREATE MATERIALIZED VIEW ER_VISITS AS
SELECT
  T2.encounter_id,
  T2.patient,
  T2.encounterclass,
...
CREATE MATERIALIZED VIEW OP_VISITS AS 
...
CREATE MATERIALIZED VIEW all_visits AS
SELECT
  *,
  ROW_NUMBER() OVER(
    ORDER BY
...
CREATE MATERIALIZED VIEW assign_all_visit_ids AS
SELECT
...
CREATE MATERIALIZED VIEW final_visit_ids AS
